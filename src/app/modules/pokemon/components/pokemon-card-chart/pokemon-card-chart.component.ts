...
} from 'src/app/utils/chart/bar-chart';

@Component({
  selector: 'pokemon-card-chart',
...
export class PokemonCardChartComponent {
  ...
  set currentPokemon(currentPokemon: Pokemon) {
    ...
    if (currentPokemon) {
      this.fillPokemonChartData(
        currentPokemon,
        this.comparisonPokemonInformation
      );
    }
  }
  ...
  set comparisonPokemon(comparisonPokemon: Pokemon) {
    ...
    if (comparisonPokemon) {
      this.fillPokemonChartData(
        this.currentPokemonInformation,
        comparisonPokemon
      );
    }
  }
  ...
  fillPokemonChartData(
    ...
    if (this.isComparing) {
      const comparisonPokemonBaseStat = comparisonPokemon.stats.map(
        (stat) => stat.base_stat
      );
